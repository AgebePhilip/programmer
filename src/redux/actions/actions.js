import { getUrl, getHttp, postHttp, getFetchHttp, postFetchHttp } from '../../common/WooComFetch'
...
export function logOut (dispatch, th) {
  dispatch(async dispatch => {
    th.setState({
      spinnerTemp: false
    })
...
export function signIn (dispatch, emailSignIn, passwordSignIn,
  sessionId, th, languageId, currencyId) {
  dispatch(async dispatch => {
    const dat = {}
    ...
    if (json.status === 'success' && json.data.data !== null) {
...
export function signUp (dispatch, firstNameSignUp,
  ...
  dispatch(async dispatch => {
    ...
    signIn(dispatch, emailSignUp, confirmPasswordSignUp,
      sessionId, th)
    dispatch({
      type: REGISTER_USER,
      payload: json.data,
...
export function checkCouponAvalability (dispatch, value, th, languageId, currencyId) {
  dispatch(async dispatch => {
...
export function getCartProducts (dispatch, sessionId, th, languageId, currencyId) {
  dispatch(async dispatch => {
    ...
    th.props.cartProductsArray.forEach(async (value, index) => {
      ...
      count++
      const p = data.data
      if (p.id === undefined) { th.props.cartProductsArray.splice(index, 1) } else if (p.status === 'trash') { th.props.cartProductsArray.splice(index, 1) } else {
        value.price = p.price
        value.name = p.name
        ...
        if (p.stock_quantity === null) { } else if (p.stock_quantity < value.quantity) value.quantity = p.stock_quantity
        else if (p.stock_status !== 'instock') { th.props.cartProductsArray.splice(index, 1) }
...
export async function checkProductStock (id, type, combinationId = null, quantity) {
...
export function getCurrency (dispatch) {
  dispatch(async dispatch => {
    const data = await postFetchHttp(ThemeStyle.url + '/wp-json/api/tc_settings/app_all_currencies', {})
    dispatch({
      type: GET_CURRENCY,
...
export function aboutUsData (dispatch, temp) {
  dispatch(async dispatch => {
    let url = ThemeStyle.url + '/wp-json/wp/v2/pages/'
    if (temp.about_page_id !== undefined) {
      const ids = temp.about_page_id + ',' +
        temp.refund_page_id + ',' +
        temp.terms_page_id + ',' +
...
export function getVendorList (dispatch, temp) {
  dispatch(async dispatch => {
    ...
    if (temp.mvf_enabled === '1') {
      ...
      const url = ThemeStyle.url + '/wp-json/api/tc_settings/app_featured_dokan_vendors_list'
      ...
      const url = ThemeStyle.url + '/wp-json/api/tc_settings/app_featured_wcvendors_list'
      ...
      if (data.status === 'success') {
        ...
        for (const d of ddata) {
          if (d.vendor_banner) { d.banner = d.vendor_banner } else { d.banner = '' }
...
export function firstSettingCallFun (dispatch, props, th) {
  dispatch(async dispatch => {
    const settings = await getFetchHttp(ThemeStyle.url + '/wp-json/api/tc_settings/app_all_settings', {})
    let temp = {}
    temp = settings.data
    ...
    getfeaturedProducts(dispatch, props.languageCode, props.currencyCode, 1, th)

    gettopsellerProducts(dispatch, props.languageCode, props.currencyCode, 1, th)
...
export function getBanners (dispatch) {
  dispatch(async dispatch => {
    const json = await getFetchHttp(ThemeStyle.url + '/wp-json/api/tc_settings/app_all_banners', {})
    if (json.status === 'success') {
      dispatch({
        type: GET_BANNERS,
...
export function getProducts (dispatch, languageId, currencyId, page, category, th) {
  dispatch(async dispatch => {
    const data = await getHttp('products/', {
      lang: languageId,
      currency: currencyId,
...
export function getcartRelatedProducts (dispatch, page, languageCode, currencyCode) {
  dispatch(async dispatch => {
    const data = await getHttp('products/', {
      lang: languageCode,
      currency: currencyCode,
...
export function getCategories (dispatch, languageCode, currencyCode,
  page = 1, th) {
  dispatch(async dispatch => {
    const perPage = 99
    ...
    if (data.status === 'success') {
      if (data.status === 'success') {
        if (data.data.length === perPage) {
          getCategories(
            dispatch, languageCode, currencyCode, ++page, th)
        }
