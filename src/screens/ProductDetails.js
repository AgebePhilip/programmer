...
class ProductDetail extends PureComponent {
  ...
  constructor (props) {
    ...
    this.state = {
      ...
      quantityNumber: this.props.navigation.state.params.objectArray.type === 'grouped' ? 0 : 1,
      priceNumber: 0,
  ...
  componentDidMount () {
    ...
    // get relatedProducts
    this.props.getRelatedProductsFun(this.props, this.state.page,
      this.props.navigation.state.params.objectArray.related_ids
    )
    if (this.props.navigation.state.params.objectArray.type === 'grouped') { this.getGroupedProducrs() }
  }

  getWcVendorInfo = async () => {
    const data = await postFetchHttp(ThemeStyle.url + '/wp-json/api/tc_settings/app_vendor_info/?product_id=' + this.props.navigation.state.params.objectArray.id, { product_id: this.props.navigation.state.params.objectArray.id })
  ...
  initializeAllVariationAttributes (p) {
    ...
    ob.id = p.id
    ob.select = false
    for (const val of this.props.navigation.state.params.objectArray.attributes) {
  ...
  sortAttributes () {
    ...
    for (const x of this.state.selectAttributes) {
      for (const y of this.state.tempAllVariableAttributes) {
        ...
        if (removeDiacritics(y[keyOfY]) === removeDiacritics(x.value) || y[keyOfY] === 'any') {
          if (count === 0) { y.select = true } else {
  ...
  availableOption (name, val) {
    ...
    for (const value of this.state.tempAllVariableAttributes) {
      if (value.select === true) {
        ...
        if (removeDiacritics(value[name]) === removeDiacritics(val)) return true
      }
    }
  }

  getGroupedProducrs = async () => {
    if (this.props.navigation.state.params.objectArray.grouped_products.length > 0) {
      this.setState({ spinnerTemp: true })
      const data = await getHttp('products/', {
        lang: this.props.languageCode,
        currency: this.props.currencyCode,
        include: this.props.navigation.state.params.objectArray.grouped_products,
        status: 'publish'
      })
      if (data.status === 'success') {
        if (data.status === 'success') {
          data.data.forEach(element => {
            this.state.groupeProductArray.push(Object.assign(element, { quantity: 0 }))
          })
          this.setState({
  ...
  checkProductNew = () => {
    const pDate = new Date(this.props.navigation.state.params.objectArray.date_created)
    const date =
      pDate.getTime() +
  ...
  calculatePrice = () => {
    if (this.state.priceNumber !== 0) { return this.state.quantityNumber * this.state.priceNumber } else return 0
  }
  ...
  addVaration = (attribute, id, name) => {
    ...
    this.setState({ selectedVariationsArray: array, enableOutOFStockButtonBool: false }, () => {
      if (array.length === this.props.navigation.state.params.objectArray.attribute.length) {
  ...
  findAndSelectProductCombination = () => {
    let found = 0
    this.props.navigation.state.params.objectArray.product_combination.forEach(combination => {
      found = 0
  ...
  getWishListId = (id) => {
    ...
    if (this.props.wishlistArray !== undefined &&
      ...
      this.props.wishlistArray !== '') {
      this.props.wishlistArray.forEach(element => {
        if (element === id) { found = element.wishlist }
      })
    }
    return found
  }
  ...
  addWishlist = (props, t, data) => {
    if (this.productIsInList(data.id)) {
      ...
      this.props.removeWishlistCall(this.props.userData, data.id,
        this, wishListId)
      Snackbar.show({
      ...
      const data2 = this.props.storeWishlistCall(this.props.userData, data.id,
        this)
      Snackbar.show({
        backgroundColor: this.props.themeStyle.primary,
        textColor: this.props.themeStyle.textTintColor,
    ...
    if (this.props.products.length % 10 === 0) {
      this.setState(
        ...
        () => {
          this.state.page++
          this.props.getRelatedProductsFun(this.props, this.state.page,
            this.props.navigation.state.params.objectArray.category[0].category_detail.detail[0].category_id)
        }
      )
    } else {
      this.setState({
  ...
  //= ==============================================================================================================================
  qunatityGroupPlus (p) {
    if (p.stock_quantity == null || p.stock_quantity > p.quantity) p.quantity = p.quantity + 1
  ...
  openStore = () => {
    if (this.props.settings.mvf_enabled === '1') {
      ...
      let id
      if (this.props.navigation.state.params.objectArray.store.ID) id = this.props.navigation.state.params.objectArray.store.ID
      if (this.props.navigation.state.params.objectArray.store.id) id = this.props.navigation.state.params.objectArray.store.id
      if (this.props.navigation.state.params.objectArray.store.user_id) id = this.props.navigation.state.params.objectArray.store.user_id
      this.props.navigation.navigate('VendorScreen', {
        data: this.props.navigation.state.params.objectArray.store,
        id: id
      })
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center' }}>
        <Text
          style={{
            ...
            color: this.props.themeStyle.textColor,
            paddingTop: 8
          }}>{this.props.language['Sold by'] + ' '}</Text>
        <Text
          style={{
            textAlign: 'left',
            fontSize: appTextStyle.mediumSize,
  ...
  render () {
    ...
    return this.state.activityIndicatorTemp ? (
      ...
      <View style={[styles.container, { backgroundColor: this.props.themeStyle.primaryBackgroundColor }]}>
        <Header shadow={false} backIcon={true} cartIcon={true} navigation={this.props.navigation} name={this.props.language['Product Details']} />
              ...
              <View
                ...
                <View style={styles.modalDetailContainer}>
                  {data.images[0].src !== undefined
                    ? <ImageLoad
                      ...
                      source={this.state.imageUrlString === ''
                ...
                </View>
                {
                  this.props.navigation.state.params.objectArray.attributes !== undefined
                    ? this.props.navigation.state.params.objectArray.attributes.map((att, ind) => (
                            ...
                            this.availableOption(att.name, attValue) ? (
                              <TouchableOpacity
                                ...
                                style={[styles.attributeView, {
                                  backgroundColor: this.selectedBadge(att, att.id) ? this.props.themeStyle.primary : this.props.themeStyle.secondryBackgroundColor,
                                  borderColor: this.selectedBadge(att, att.id) ? this.props.themeStyle.primary : this.props.themeStyle.iconPrimaryColor
                                }]}
                                onPress={() => this.fillAttributes(attValue, att.name, ind)}
                              >
                                <Text style={[{
                                  color: !this.selectedBadge(att, att.id) ? this.props.themeStyle.textColor : this.props.themeStyle.textTintColor,
                                  fontSize: appTextStyle.largeSize,
                                  fontFamily: appTextStyle.fontFamily
                                }]}>
                                  {attValue}
                ...
                {this.state.stockIndicator
                  ...
                  : <TouchableOpacity
                    ...
                    onPress={() => {
                      this.props.addToCartCall(data, this.state.selectedVariation,
                        this.state.quantityNumber,
                        this.state.selectAttributes, this)
                      this.resetAttributes()
                    }}
                    disabled={this.state.selectAttributes.length !== this.props.navigation.state.params.objectArray.attributes.length}
                  >
                    <View
                      style={[styles.btnView, {
                        width: WIDTH * 0.8,
                        backgroundColor: this.state.selectAttributes.length !== this.props.navigation.state.params.objectArray.attributes.length ? 'gray' : this.props.themeStyle.primary
                      }]}>
                      <Icon
                        name={'cart'}
          ...
          ListHeaderComponent={
              ...
              <View style={[styles.headerBar, { width: WIDTH }]}>
                ...
                <TouchableOpacity onPress={() => {
                  ...
                  <Ionicons
                    ...
                    active
                    name={this.productIsInList(this.props.navigation.state.params.objectArray.id) ? 'heart' : 'heart-outline'}
              ...
              <View style={[styles.labelsContainer, {
                ...
                <View style={[styles.labelRow]}>
                  <View style={styles.labelsView}>
                    ...
                    {
                      data.featured
                        ? this.labelesFun(this.props.themeStyle.primary, this.props.language.FEATURED)

                        : <View />}
                    {
                      this.checkProductNew()
                        ? this.labelesFun('green', this.props.language.NEW) : <View />}
            ...
            node.children[0] !== null
              ...
              {this.props.navigation.state.params.objectArray.store !==
                  undefined &&
                this.props.settings.mvf_enabled === '1' ? (
                  this.sellerInfo(this.props.navigation.state.params.objectArray.store.name)
                ) : null}
              {this.state.wcVendorInfo.display_name !==
                  undefined &&
              ...
              {this.props.navigation.state.params.objectArray.store !==
                  undefined &&
                this.props.settings.mvf_enabled === '3' ? (
                  this.sellerInfo(this.props.navigation.state.params.objectArray.store.vendor_display_name)
                ) : null}
              ...
              <TouchableOpacity style={[styles.productDetailsView, {
                backgroundColor: this.props.themeStyle.secondryBackgroundColor
              }]}
              onPress={() => {
              ...
              >
                <View
                  ...
                  <View style={styles.reviewContainer}>
                    ...
                    <Text style={[styles.priceText, {
                      ...
                      {'(' + this.props.navigation.state.params.objectArray.rating_count + ')'}
                    </Text>
                  </View>
                  <Text style={[styles.priceText, {
                    color: this.props.themeStyle.textColor,
                ...
                {this.props.navigation.state.params.objectArray.average_rating !== undefined &&
                  this.props.navigation.state.params.objectArray.average_rating !== null
                  ? <View style={styles.avgReviewContainer}>
                    <Text style={[styles.priceText, {
                      ...
                      {this.props.navigation.state.params.objectArray.average_rating}
                    </Text>
              ...
              <TouchableOpacity
                style={styles.addtoCartBtn}
                onPress={() => {
                  if (this.props.navigation.state.params.objectArray.external_url !== '' &&
                    this.props.navigation.state.params.objectArray.external_url !== undefined &&
                    this.props.navigation.state.params.objectArray.external_url !== null) {
                    this.externalLink(
                  ...
                  if (data.type === 'variable') {
                    ...
                    this.setState({ spinnerTemp: true }, async () => {
                      this.props.addToCartCall(data, null,
                        this.state.quantityNumber,
                        null, this)
                    })
                    // }
                    ...
                    if (total === 0) {
                      ...
                      for (const value of this.state.groupeProductArray) {
                        if (value.quantity > 0) {
                          this.props.addToCartCall(value, null,
                            value.quantity,
                            null, this)
                        }
                      }
...
const mapDispatchToProps = dispatch => ({
  ...
  getRelatedProductsFun: (props, page, selectedItem) => {
    dispatch(async dispatch => {
      await getpdRelatedProducts(dispatch, props.languageCode, props.currencyCode, page, selectedItem)
    })
  },
  clearProducts: () => {
    dispatch({
  ...
  addToCartCall: (product, selectedVariation, quantityNumber, selectAttributes, th) => {
    dispatch(dispatch => {
      addToCartFun(dispatch, product, selectedVariation, quantityNumber,
        selectAttributes, th)
    })
  }
})
